...
Panel {
  ...
  property bool healthModeActive: false
  ...
  readonly property bool chargeThresholdActive: {
    var device = UPower.displayDevice
    return Model.chargeThresholdActive(device, root.discharging, upowerStates()) || (root.healthModeActive && !root.discharging && batteryFraction >= 0.79)
  }
  ...
  function refresh() {
    ...
    if (!healthModeProc.running) healthModeProc.running = true
  }

  function setHealthMode(enabled) {
    if (setHealthProc.running) return
    setHealthProc.command = ["acer-battery-limit", enabled ? "on" : "off"]
    setHealthProc.running = true
  }
  ...
  Process {
    id: healthModeProc
    command: ["acer-battery-limit", "status"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        root.healthModeActive = (text.trim() === "on")
      }
    }
  }
  ...
  BarIconButton {
    ...
    onPressed: function(b) {
      ...
      else if (b === Qt.MiddleButton) root.setHealthMode(!root.healthModeActive)
      else root.toggle()
    }
  }
        ...
        Column {
          ...
          Row {
            ...
            Button {
              ...
              verticalPadding: Style.spacing.controlPaddingY + Style.space(2)
              bordered: true
              active: root.healthModeActive
              onClicked: root.setHealthMode(true)
            }
            ...
            Button {
              ...
              bordered: true
              active: !root.healthModeActive
              onClicked: root.setHealthMode(false)
            }
          }
        }
      }
